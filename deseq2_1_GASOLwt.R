...
setwd("C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/GASOL_WTcomtextfiles/")
directory <- "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/GASOL_WTcomtextfiles/"
...
OGPCAN_matrix$Condition <- c(rep("GF", 2), rep("SF", 7)) 
...
#---------MA plot from res--------------------------------------------------------
plotMA.SOL_GA_W <- plotMA(res.SOL_GA_W, ylim=c(-2,2))
mcols(res.SOL_GA_W, use.names = TRUE)
...
res.SOL_GA_W_filtered <- as.data.frame(as.matrix(subset(res.SOL_GA_W, pvalue < 0.05)))
res.SOL_GA_W_filtered <- as.data.frame(as.matrix(subset(res.SOL_GA_W, padj < 0.05)))
res.SOL_GA_W_filtered$absFC <- abs(res.SOL_GA_W_filtered$log2FoldChange)

#print(res.TA_GA_M_filtered)
#dim(res.SOL_GA_W_filtered)

write.table(res.SOL_GA_W_filtered, "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/RNAseq_analysis/res.SOL_GA_W_filtered_20180626.txt", sep ="\t")
#write.csv(as.data.frame(res.SOL_GA_WW_filtered), file = "")

#----for Heatmap plots of 200 gene expressions and significant Log2fold changes-----
#nrow(res.SOL_GA_W_filtered)
#ncol(res.SOL_GA_W_filtered) 
#head(res.SOL_GA_W_filtered)
...
SOL_GA_W.heatmap.matrix <- as.matrix(res.SOL_GA_W_filtered[ ,c(2)])
#class(res.SOL_GA_W_filtered)
#class(SOL_GA_W.heatmap.matrix)
#print(SOL_GA_W.heatmap.matrix)
#SOL_GA_W.heatmap.matrix <- t(SOL_GA_W.heatmap.matrix) # flip rows and columns around
Heatmap(SOL_GA_W.heatmap.matrix)
Heatmap(SOL_GA_W.heatmap.matrix, cluster_columns = F) #Formats based on alphabetical order
...
var_genes <- apply(res.SOL_GA_W_filtered, 1, var)
head(var_genes)
