...
setwd("C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/mutant/TAvsSol/")
directory <- "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/mutant/TAvsSol/"

#-----Set up DESeq2 data, based on names of HTSeq counts in working directory-----
sampleFiles <- dir(pattern = 'sorted')
#print(sampleFiles)
sampleIdentifiers<-c("S1", "S2", "S3", "S4", "S5", "T1", "T2", "T3", "T4")
...
mds$condition <- c("Soleus", "Soleus","Soleus","Soleus","Soleus", "Tibialis", "Tibialis","Tibialis", "Tibialis")
...
mdsPois$condition <- c("Soleus", "Soleus","Soleus","Soleus","Soleus", "Tibialis", "Tibialis","Tibialis", "Tibialis")
...
grp <- c("Soleus", "Soleus", "Soleus", "Soleus", "Soleus", "Tibialis", "Tibialis", "Tibialis", "Tibialis")
...
#print(OGPCAN_matrix)
OGPCAN_matrix$Condition <- c("Soleus","Soleus","Soleus","Soleus","Soleus","Tibialis","Tibialis","Tibialis", "Tibialis")
#print(OGPCAN_matrix)
...
res.TA_SOL_M <- results(ddsHTSeqFiltered, contrast = c("condition", "S", "TM"))
#summary(res.TA_SOL_M)
...
plotMA.SOL_GA_C <- plotMA(res.TA_SOL_M, ylim=c(-3,3))


##---Volcano Plots----------------------------------------------------------------
#-Unlabelled plot----
with(res.TA_SOL_M, plot(log2FoldChange, -log10(pvalue), 
                        pch=10,
                        xlab=expression(paste("log"[2]*Delta,"FC (TA/SOL)")), 
                        ylab=expression(paste("-log"[10]*"(p-value)"))))

#-Add colored points: blue if padj<0.05, red if log2FC>1, green if both
with(subset(res.TA_SOL_M, padj<0.05), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="light grey"))
with(subset(res.TA_SOL_M, abs(log2FoldChange)>1), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="dark grey"))
with(subset(res.TA_SOL_M, padj<0.05 & abs(log2FoldChange)>1), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="yellow"))
with(subset(res.TA_SOL_M, padj<0.05 & log2FoldChange < (-1)), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))

#-Label points with the textxy function from the calibrate plot
with(subset(res.TA_SOL_M),
     identify(log2FoldChange, -log10(pvalue), labels=rownames(res.TA_SOL_M),
              cex=0.6))
...
res.TA_SOL_M.05 <- results(ddsHTSeqFiltered, alpha=0.05)
#table(res.TA_SOL_M.05$padj < 0.05)

#-Calculate differentially expressed genes from DESeq2 object 
# based on log fold change equal to 0.5 (2^0.5)
res.TA_SOL_MLFC1 <- results(ddsHTSeqFiltered, lfcThreshold=0.5)
#table(res.TA_SOL_MLFC1$padj < 0.1)

#-Subset data based on (1) adjusted p-value less than 0.05 AND 
# (2) absolute value of the log2 fold change greater than 0.5
res.TA_SOL_M_filtered2 <- subset(res.TA_SOL_M, padj < 0.05)
res.TA_SOL_M_filtered2$absFC <- abs(res.TA_SOL_M_filtered2$log2FoldChange)
#head(res.TA_SOL_M_filtered2)
#dim(res.TA_SOL_M_filtered2)

res.TA_SOL_M_filtered3 <- subset(res.TA_SOL_M_filtered2, absFC > 1)
#head(res.TA_SOL_M_filtered3)
#dim(res.TA_SOL_M_filtered3)

#---Print out the filtered data as a text file---
write.table(res.TA_SOL_M_filtered2, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_SOL_M_filtered_padj_20180918.txt", 
            sep ="\t")
write.table(res.TA_SOL_M_filtered3, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_SOL_M_filtered_padjfoldchange_20180918.txt", 
            sep ="\t")
write.table(res.TA_SOL_M, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_SOL_M_20180918.txt", 
            sep = "\t")
...
Mat <- assay(vsd)[order(res.TA_SOL_M_filtered2$padj), ]
