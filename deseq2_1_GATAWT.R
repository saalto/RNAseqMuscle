...
setwd("C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/control/GAvsTA/")
directory <- "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/control/GAvsTA/"

#-----Set up DESeq2 data, based on names of HTSeq counts in working directory-----
sampleFiles <- dir(pattern = 'sorted')
#print(sampleFiles)

sampleIdentifiers <- c("G1", "G2", "T1", "T2")
...
mds$condition <- c("Gastrocnemis", "Gastrocnemis", "Tibialis", "Tibialis")
...
mdsPois$condition <- c("Gastrocnemis", "Gastrocnemis", "Tibialis", "Tibialis")
...
grp <- c("Gastrocnemius", "Gastrocnemius", "Tibialis", "Tibialis")
...
res.TA_GA_C <- results(ddsHTSeqFiltered, contrast = c("condition", "GF", "TF"))
#summary(res.TA_GA_C)
...
plotMA.SOL_GA_C <- plotMA(res.TA_GA_C, ylim=c(-3,3))


##---Volcano Plots----------------------------------------------------------------
#-Unlabelled plot----
with(res.TA_GA_C, plot(log2FoldChange, -log10(pvalue), 
                       pch=10,
                       xlab=expression(paste("log"[2]*Delta,"FC (GA/TA)")), 
                       ylab=expression(paste("-log"[10]*"(p-value)"))))

#-Add colored points: blue if padj<0.05, red if log2FC>1, green if both
with(subset(res.TA_GA_C, padj<0.05), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="light grey"))
with(subset(res.TA_GA_C, abs(log2FoldChange)>1), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="dark grey"))
with(subset(res.TA_GA_C, padj<0.05 & abs(log2FoldChange)>1), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="yellow"))
with(subset(res.TA_GA_C, padj<0.05 & log2FoldChange < (-1)), 
     points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))

#-Label points with the textxy function from the calibrate plot
with(subset(res.TA_GA_C),
     identify(log2FoldChange, -log10(pvalue), labels=rownames(res.TA_GA_C),
              cex=0.6))
...
res.TA_GA_C_filtered2$absFC <- abs(res.TA_GA_C_filtered2$log2FoldChange)
#head(res.TA_GA_C_filtered2)
#dim(res.TA_GA_C_filtered2)

res.TA_GA_C_filtered3 <- subset(res.TA_GA_C_filtered2, absFC > 1)
#head(res.TA_GA_C_filtered3)
#dim(res.TA_GA_C_filtered3)

#---Print out the filtered data as a text file---
write.table(res.TA_GA_C_filtered2, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_GA_C_filtered_padj_20180918.txt", 
            sep ="\t")
write.table(res.TA_GA_C_filtered3, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_GA_C_filtered_padjfoldchange_20180918.txt", 
            sep ="\t")
write.table(res.TA_GA_C, 
            "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/iteration2/RNAseq_analysis/res.TA_GA_C_20180918.txt", 
            sep = "\t")
