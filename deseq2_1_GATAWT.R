...
setwd("C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/GATA_WTcomtextfiles/")
directory <- "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/GATA_WTcomtextfiles/"
...
OGPCAN_matrix$Condition <- c(rep("GF", 2), rep("TF", 3)) 
...
#---------MA plot from res--------------------------------------------------------
plotMA.TA_GA_W <- plotMA(res.TA_GA_W, ylim=c(-2,2))
mcols(res.TA_GA_W, use.names = TRUE)
...
res.TA_GA_W_filtered <- as.data.frame(as.matrix(subset(res.TA_GA_W, pvalue < 0.05)))
res.TA_GA_W_filtered <- as.data.frame(as.matrix(subset(res.TA_GA_W, padj < 0.05)))
res.TA_GA_W_filtered$absFC <- abs(res.TA_GA_W_filtered$log2FoldChange)

#print(res.TA_GA_W_filtered)
#dim(res.TA_GA_W_filtered)

write.table(res.TA_GA_W_filtered, "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/RNAseq_analysis/res.TA_GA_W_filtered_20180626.txt", sep ="\t")
#write.csv(as.data.frame(res.TA_GA_W_filtered), file = "")

#----for Heatmap plots of 200 gene expressions and significant Log2fold changes-----
#nrow(res.TA_GA_W_filtered)
#ncol(res.TA_GA_W_filtered) 
#head(res.TA_GA_W_filtered)
...
TA_GA_W.heatmap.matrix <- as.matrix(res.TA_GA_W_filtered[ ,c(2)])
#class(res.TA_GA_W_filtered)
#class(TA_GA_W.heatmap.matrix)
#print(TA_GA_W.heatmap.matrix)
#TA_GA_W.heatmap.matrix <- t(TA_GA_W.heatmap.matrix) # flip rows and columns around
Heatmap(TA_GA_W.heatmap.matrix)
Heatmap(TA_GA_W.heatmap.matrix, cluster_columns = F) #Formats based on alphabetical order
...
var_genes <- apply(res.TA_GA_W_filtered, 1, var)
head(var_genes)
