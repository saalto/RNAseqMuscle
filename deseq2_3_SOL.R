...
OGPCAN_matrix$Condition <- c(rep("FlZ", 7), rep("MCK", 7)) 
...
res.SOL_W_M_filtered2 <- subset(res.SOL_W_M_filtered, absFC > 0.5)
head(res.SOL_W_M_filtered2)
dim(res.SOL_W_M_filtered2)
res.SOL_W_M_filtered3 <- subset(res.SOL_W_M_filtered2, padj < 0.05)
...
write.table(res.SOL_W_M_filtered, "C:/Users/sarah/OneDrive/Documents/2018/03_2018_Summer/RNAseq_analysis/res.SOL_W_M_filtered_20180626.txt", sep ="\t")
...
#nrow(res.SOL_W_M_filtered)
#ncol(res.SOL_W_M_filtered) 
#head(res.SOL_W_M_filtered)
...
#head(res.SOL_W_M_filtered)
SOL_W_M.heatmap.matrix <- as.matrix(res.SOL_W_M_filtered[ ,c(2)])
#class(res.SOL_W_M_filtered)
#class(SOL_W_M.heatmap.matrix)
#print(SOL_W_M.heatmap.matrix)
#SOL_W_M.heatmap.matrix <- t(SOL_W_M.heatmap.matrix) # flip rows and columns around
Heatmap(SOL_W_M.heatmap.matrix)
Heatmap(SOL_W_M.heatmap.matrix, cluster_columns = F) #Formats based on alphabetical order
...
var_genes <- apply(res.SOL_W_M_filtered, 1, var)
head(var_genes)
